...
from happyhours.factories import UserFactory, CategoryFactory, BeverageFactory, EstablishmentFactory
...
class TestBeverageViewSetPermissions:
    ...
    def test_create_beverage_permissions(self):
        ...
        response = self.client.post(self.list_url,
                                    {'name': 'New Beverage', 'price': '5.99', 'description': "some text",
        ...
        self.client.force_authenticate(user=self.regular_user)
        response = self.client.post(self.list_url, {'name': 'Another Beverage', 'price': '6.99'})
        assert response.status_code == status.HTTP_403_FORBIDDEN
