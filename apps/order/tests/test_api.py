...
from happyhours.factories import UserFactory, BeverageFactory, EstablishmentFactory, OrderFactory
...
class TestOrderViews:
    def setup_method(self):
        ...
        self.establishment = EstablishmentFactory(owner=self.partner, happyhours_start='01:00:00',
    ...
    def test_partner_order_history_data(self):
        self.client.force_authenticate(user=self.partner)
        response = self.client.get(self.partner_order_history_url)
        assert len(response.data) == Order.objects.filter(establishment__owner=self.partner).count()
